...
def get_bunker_db():
    if 'bunker_db' not in g:
        g.bunker_db = sqlite3.connect(current_app.config['BUNKER_DATABASE'], timeout=10)
        g.bunker_db.execute('PRAGMA journal_mode=WAL')
...
def init_db():
    ...
    db.execute("CREATE INDEX IF NOT EXISTS idx_maquinas_autor ON maquinas(autor)")
    db.execute("CREATE INDEX IF NOT EXISTS idx_maquinas_dificultad ON maquinas(dificultad)")
    db.execute(
        """
