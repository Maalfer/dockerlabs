...
async def api_auth_register(request: Request, data: RegisterRequest):
    ...
    if auth_conflict:
        ...
        from dockerlabs.models import EmailVerificationToken
        ...
        if is_smtp_configured():
            try:
                ...
                alchemy_db.session.add(pending)
                alchemy_db.session.commit()
            ...
            _logging.getLogger(__name__).warning("SMTP no configurado — creando usuario sin verificacion de email")
            try:
...
async def api_auth_reset_password(request: Request, data: ResetPasswordRequest):
    ...
    if datetime.utcnow() > reset_token.expires_at:
        alchemy_db.session.delete(reset_token)
        alchemy_db.session.commit()
...
async def api_upload_machine_logo(
    ...
    try:
        ...
        if maq.logo_path:
            old_path = _os.path.join(BASE_DIR, maq.logo_path)
