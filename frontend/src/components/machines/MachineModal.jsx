...
export default function MachineModal({ open, data, onClose }){
  ...
  const imgUrl = data.imagen ? `/assets/${data.imagen.replace(/^\/+/, '')}` : '/assets/dockerlabs/images/logos/logo.png'
  ...
  return (
    <div className={`machine-modal-overlay ${visible? 'visible' : ''}`} onClick={(e)=>{ if (e.target === e.currentTarget) close() }}>
      <div className={`machine-modal-popup ${visible? 'visible' : ''}`} style={ data.color ? { border: `2px solid ${data.color}`, boxShadow: `0 0 20px -5px ${data.color}, 0 25px 50px -12px rgba(0,0,0,0.5)` } : {} }>
        ...
        <div className="popup-content">
          ...
          <div className="creator-card">
            <img src={author.profile_image_url || '/assets/dockerlabs/images/balu.webp'} className="creator-avatar" alt="avatar" />
        ...
        {showRatingModal && (
          <div className="machine-modal-overlay visible" style={{zIndex:11000}} onClick={(e)=>{ if (e.target === e.currentTarget) setShowRatingModal(false) }}>
