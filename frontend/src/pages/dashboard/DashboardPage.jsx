...
export default function DashboardPage() {
  ...
  /* ── fetch data ─────────────────────────────────────── */
  /* ── fetch data ─────────────────────────────────────── */
  // User is already loaded by AuthContext
  ...
  useEffect(() => {
    ...
    if (!authLoading && !authUser?.is_authenticated) {
      navigate('/login')
    }
  ...
  // Pre-fill form states when user loads (if not already modified)
  // We use a separate useEffect to sync local state with auth context 
  // ensuring we don't overwrite user input if they are typing
  useEffect(() => {
    if (me) {
      setBio(prev => prev || me.biography || '')
      setLinkedin(prev => prev || me.linkedin_url || '')
      setGithub(prev => prev || me.github_url || '')
      setYoutube(prev => prev || me.youtube_url || '')
    }
  }, [me.username]) // Sync when user changes
  /* ── photo upload ───────────────────────────────────── */
  ...
  /* ── render ─────────────────────────────────────────── */
  /* ── render ─────────────────────────────────────────── */
  if (isLoading) return <div className="dashboard-container"><p>Cargando dashboard...</p></div>
  if (!authUser?.is_authenticated) return null
          ...
          <div className="profile-content">
            ...
            <div className="profile-form">
              ...
              <div className="form-actions">
                ...
                <button className="btn-cancel" onClick={refreshUser}>
