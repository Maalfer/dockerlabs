...
let _plusMenuClickHandler = null;
...
function rankingEquipos() {
    ...
    // Close menu when clicking outside — remove previous listener before adding a new one
    if (_plusMenuClickHandler) {
        document.removeEventListener('click', _plusMenuClickHandler);
    }
    _plusMenuClickHandler = (e) => {
        const dropdown = document.getElementById('plus-dropdown-menu');
    ...
    document.addEventListener('click', _plusMenuClickHandler);
...
function renderMyTeam(container, team, popup) {
    ...
    if (popup) {
        ...
        if (!document.getElementById('plus-menu-btn')) {
            ...
            editItem.className = 'plus-dropdown-item';
            editItem.innerHTML = '<i class="bi bi-pencil"></i><span>Editar equipo</span>';
            editItem.addEventListener('click', () => { closePlusMenu(); showEditarEquipoForm(team.id); });
    ...
    const amCreator = team.is_creator === true;
        ...
        <div class="member-item" style="cursor: pointer;" onclick="openAuthorProfileModal('${escapeHtml(member.username)}')">
            ...
            <div class="member-info">
                <div class="member-name">
                    ...
                    ${member.is_creator ? '<span style="color: #f59e0b; font-size: 0.7rem;"> 👑</span>' : ''}
                </div>
                <div class="member-puntos">${member.puntos} pts</div>
            </div>
            ${amCreator && !member.is_me ? `
            ...
            ` : ''}
            ${amCreator && member.is_me && team.member_count > 1 ? `
                <button class="member-remove-btn" onclick="event.stopPropagation(); showTransferirLiderazgoForm(${team.id})" title="Transferir liderazgo" style="color: #f59e0b;">
...
function switchTabDetail(btn, tabId, teamId) {
    // Update active tab
    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    ...
    // Show/hide content
    document.querySelectorAll('.tab-content').forEach(c => c.style.display = 'none');
    const content = document.getElementById('tab-' + tabId + '-detail');
    if (content) content.style.display = 'block';

    // Load solicitudes when switching to solicitudes tab
    if (tabId === 'solicitudes' && teamId) {
        loadSolicitudesPendientesDetail(teamId);
    }
}

function loadSolicitudesPendientesDetail(teamId) {
    fetch(`/api/equipos/${teamId}/solicitudes`)
        .then(res => res.json())
        .then(data => {
            const container = document.getElementById('solicitudes-list-detail');
            if (!container) return;
                ...
                <div class="invitation-item">
                    ...
                    <div class="invitation-actions">
                        <button class="team-btn team-btn-primary" style="padding: 0.4rem 0.75rem; font-size: 0.8rem;"
                                onclick="responderSolicitud(${req.id}, true, ${teamId})">
                            <i class="bi bi-check-lg"></i>
                        </button>
                        <button class="team-btn team-btn-danger" style="padding: 0.4rem 0.75rem; font-size: 0.8rem;"
                                onclick="responderSolicitud(${req.id}, false, ${teamId})">
                            <i class="bi bi-x-lg"></i>
                        </button>
                    </div>
                </div>
...
function showEditarEquipoForm(teamId) {
    ...
    fetch(`/api/equipos/${teamId}`)
        ...
        .then(data => {
            ...
            popupDiv.innerHTML = `
                <div class="modal-header">
                    <h1 class="modal-title">Editar Equipo</h1>
                    <div class="modal-subtitle">${escapeHtml(team.nombre)}</div>
                </div>

                <div id="preview-container-edit" class="team-preview-container" style="display: none;">
                    <img id="preview-image-edit" class="team-preview-image">
                </div>

                <input type="text" id="teamNombreEdit" class="team-input" placeholder="Nombre del equipo..." maxlength="50" value="${escapeHtml(team.nombre)}">

                <input type="file" id="teamImagenEdit" class="team-file-input" accept="image/*">
                <label for="teamImagenEdit" class="team-file-label">
                    <i class="bi bi-image"></i>
                    <span id="file-label-text-edit">Cambiar imagen del equipo</span>
                </label>

                <button class="team-btn team-btn-primary" onclick="editarEquipo(${team.id})" style="width: 100%;">
            ...
            // Setup image preview
            const fileInput = popupDiv.querySelector('#teamImagenEdit');
            const previewContainer = popupDiv.querySelector('#preview-container-edit');
            const previewImage = popupDiv.querySelector('#preview-image-edit');
            const fileLabel = popupDiv.querySelector('#file-label-text-edit');

            // Show current image if exists
            if (team.imagen_url) {
                previewImage.src = team.imagen_url;
        ...
        .catch(err => {
            ...
            alert('Error al cargar información del equipo');
        });
}

async function editarEquipo(teamId) {
    const nombre = document.getElementById('teamNombreEdit').value.trim();
    const imagenInput = document.getElementById('teamImagenEdit');
    ...
    try {
        const response = await fetch(`/api/equipos/${teamId}/editar`, {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            ...
            // Reopen equipos modal to show the updated team
...
function showTransferirLiderazgoForm(teamId) {
    // Fetch current team info
    fetch(`/api/equipos/${teamId}`)
        .then(res => res.json())
        .then(data => {
            ...
            const team = data.team;
            const otherMembers = team.members.filter(m => !m.is_me);

            if (otherMembers.length === 0) {
                alert('No hay otros miembros a los que transferir el liderazgo');
                return;
            }
            ...
            const membersOptions = otherMembers.map(m => 
                `<option value="${m.user_id}">${escapeHtml(m.username)}</option>`
            ).join('');

            popupDiv.innerHTML = `
                <div class="modal-header">
                    <h1 class="modal-title">Transferir Liderazgo</h1>
                    <div class="modal-subtitle">${escapeHtml(team.nombre)}</div>
                </div>

                <p style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 1rem;">
                    Selecciona un miembro para transferirle el liderazgo del equipo.
                </p>

                <select id="newLeaderSelect" class="team-input">
                    ${membersOptions}
                </select>

                <button class="team-btn team-btn-primary" onclick="transferirLiderazgo(${team.id})" style="width: 100%;">
                    <i class="bi bi-crown"></i> Transferir liderazgo
                </button>
            `;
...
async function transferirLiderazgo(teamId) {
    const newLeaderId = document.getElementById('newLeaderSelect').value;

    if (!newLeaderId) {
        alert('Selecciona un nuevo líder');
        return;
    }

    if (!confirm('¿Estás seguro de transferir el liderazgo? Perderás todos los privilegios de creador.')) {
        return;
    }

    try {
        const response = await fetch(`/api/equipos/${teamId}/transferir-liderazgo`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCsrfToken()
            },
            body: JSON.stringify({ team_id: teamId, new_leader_id: parseInt(newLeaderId) })
        });
        ...
        console.error('Error:', error);
        alert('Error al transferir liderazgo');
    }
}
    ...
    try {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                ...
                // If accepted, reopen team view to show the new team
                if (accept) {
                    setTimeout(() => openEquiposModal(), 50);
                }
            }, 300);
        } else {
            alert('Error: ' + data.message);
...
async function responderSolicitud(requestId, accept, teamId) {
    try {
        ...
        if (data.success) {
            alert(data.message);
            // Reload the correct solicitudes list depending on which modal is open
            if (teamId) {
                if (document.getElementById('solicitudes-list-detail')) {
                    loadSolicitudesPendientesDetail(teamId);
                } else if (document.getElementById('solicitudes-list')) {
                    loadSolicitudesPendientes(teamId);
                }
            }
        } else {
...
async function invitarUsuario(teamId) {
    const usernameInput = document.getElementById('invitarUsername') || document.getElementById('invitarUsernameDetail');
...
async function eliminarMiembro(teamId, userId) {
    ...
    try {
        ...
        if (data.success) {
            alert(data.message);
            // Check if we're in the main team modal or detail modal
            const isMainModal = document.querySelector('.tab-btn');
            if (isMainModal) {
                // In main team modal - refresh it
                document.querySelectorAll('.overlay.visible, .popup.visible').forEach(el => {
                    el.classList.remove('visible');
                });
                setTimeout(() => {
                ...
                setTimeout(() => openTeamDetailModal(teamId), 400);
            }
        } else {
            alert('Error: ' + data.message);
        }
    ...
    fetch(`/api/equipos/${teamId}`)
        ...
        .then(data => {
            ...
            if (team.is_member) {
                tabsHtml = `
                    <div class="tabs-container">
                        <button class="tab-btn active" onclick="switchTabDetail(this, 'miembros', ${team.id})">Miembros</button>
                        <button class="tab-btn" onclick="switchTabDetail(this, 'solicitudes', ${team.id})">Solicitudes</button>
                        ${team.member_count < team.max_members ? `
                            <button class="tab-btn" onclick="switchTabDetail(this, 'invitar', ${team.id})">Invitar</button>
                        ` : ''}
                    </div>
                `;
            }
            ...
            if (!team.is_member && team.member_count < team.max_members) {
                ...
                joinButtonHtml = `<button class="team-btn team-btn-danger" onclick="salirDelEquipo(${team.id})" style="width: 100%; margin-top: 1rem;"><i class="bi bi-box-arrow-right"></i> Salir del equipo</button>`;
            } else {
            ...
            popupDiv.innerHTML = `
                ...
                ${tabsHtml}
                <div id="tab-miembros-detail" class="tab-content">
                    ${membersHtml}
                </div>
                <div id="tab-solicitudes-detail" class="tab-content" style="display: none;">
                    <div id="solicitudes-list-detail">
                        <p style="text-align: center; color: #64748b; padding: 1rem;">Cargando solicitudes...</p>
                    </div>
                </div>
                ${team.is_member && team.member_count < team.max_members ? `
                <div id="tab-invitar-detail" class="tab-content" style="display: none;">
                    <input type="text" id="invitarUsernameDetail" class="team-input" placeholder="Nombre de usuario...">
                    <button class="team-btn team-btn-primary" onclick="invitarUsuario(${team.id})" style="width: 100%;">
                        <i class="bi bi-send"></i> Enviar invitación
                    </button>
                </div>
            ...
            // Load solicitudes if member
            if (team.is_member) {
                loadSolicitudesPendientesDetail(team.id);
            }
...
window.showEditarEquipoForm = showEditarEquipoForm;
window.showTransferirLiderazgoForm = showTransferirLiderazgoForm;
...
window.transferirLiderazgo = transferirLiderazgo;
...
window.switchTabDetail = switchTabDetail;
