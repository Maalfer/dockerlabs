function presentacion(nombre, dificultad, color, autor_nombre, autor_enlace, fecha, imagen, categoria = '', isMostRecent = false) {
    ...
    let imageUrl = "";
    if (!escImagen) {
        imageUrl = "/static/dockerlabs/images/logos/logo_120.png";
    ...
    creatorCard.innerHTML = `
        <img src="/static/dockerlabs/images/balu.webp" class="creator-avatar" id="creator-avatar-img">
    ...
    const footerVoteCount = document.createElement('span');
    footerVoteCount.id = 'modal-vote-count-display';
    footerVoteCount.style.cssText = 'font-size:0.8rem;color:#64748b;';

    footer.appendChild(footerLabel);
    footer.appendChild(footerVoteCount);
    ...
    fetch(`/api/get_machine_rating/${encodeURIComponent(nombre)}`)
        .then(res => res.json())
        .then(data => {
            ...
            if (scoreEl && data.average) {
                ...
                const voteEl = document.getElementById('modal-vote-count-display');
                if (voteEl) voteEl.textContent = data.count === 1 ? '1 voto' : `${data.count} votos`;
                ...
                const voteEl = document.getElementById('modal-vote-count-display');
                if (voteEl) voteEl.textContent = 'sin votos';
                hasUserRated = false;
                enableStarHover(footerStars);
            }
        });
                    ...
                    fetch(`/api/get_machine_rating/${machineName}`)
                        ...
                        .then(ratingData => {
                            ...
                            if (scoreEl && ratingData.average) {
                                ...
                                const voteEl = document.getElementById('modal-vote-count-display');
                                if (voteEl) voteEl.textContent = ratingData.count === 1 ? '1 voto' : `${ratingData.count} votos`;
                            }
